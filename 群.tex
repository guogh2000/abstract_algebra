...
\begin{proposition}\label{qdzhmt3}
    $G_1\oplus G_2$中的每个元素都可以分解成$\ol{G_1}$与$\ol{G_2}$中的元素的乘积,并且该分解是唯一的.
\end{proposition}
...
\end{definition}
与线性空间分解成子空间的直和的情况类似,不难证明定理\ref{asfhjbd}的条件$(2)$有以下两种等价描述
...
\begin{definition}[不可分解的]
    若群$G$不能分解成两个非平凡的正规子群的直和,则称$G$是不可分解的.
\end{definition}
事实上,任意一个有限群总可以分解成一些不可分解的群的直和.群的直和是群论中的重要问题,这里不再细说.
...
\end{proof}
\begin{remark}
    事实上,有时交换$p$-群还可被分解.以后将证明$p$-群不能被分解的充要条件为它是循环群.
\end{remark}
