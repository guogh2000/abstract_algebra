...
\begin{proof}
    \stars
\end{proof}
\begin{theorem}[Sylow第一定理-存在定理]\label{xldydl}
...
\end{proof}
\begin{definition}
    若群$G$的阶为$p^lm$,其中$(p,m)=1,p$为素数,$l\geq1$,则称$G$的$p^l$阶子群为$G$的\Sl{p}.
\end{definition}
\begin{theorem}[Sylow第二定理-包含与共轭定理]\label{xldedl}
    已知群$G$的阶为$p^lm$,其中$(p,m)=1,p$为素数,$l\geq1$.则
    
    (1)$G$的任一$p$-群包含在一\Sl{p}中.

    (2)$G$的所有\Sl{p}两两共轭,亦即$G$的所有\Sl{p}恰好构成$G$的一个共轭子群类.
\end{theorem}
\begin{proof}
    (1)
    
    由Sylow第一定理(定理\ref{xldydl})可设$G$的任一$p$-子群$H$,任一\Sl{p}$P$,令\begin{align*}
        X=\l\{gP:g\in G\r\},
    ...
    \end{align*}注意到$H,P$均成群,所以$H<g_jPg_j^{-1}$,即$H$包含在一个与$P$共轭的\Sl{p}$g_jPg_j^{-1}$中,而\begin{align*}
        |P|=|g_jPg_j^{-1}|,
    \end{align*}所以$H$包含在一\Sl{p}中,第一个论断得证.

    (2)

    当(1)中的$p$-群$H$不失任意性地取为\Sl{p}时,便有\begin{align*}
...
\begin{proof}
    ...
    设$P$是$G$的惟一一个\Sl{p},于是\begin{align}
        \forall g\in G,gPg^{-1}=P,\label{njkasdf}
    \end{align}否则由Sylow第一定理(定理\ref{xldydl})$G$有异于$P$的\Sl{p}.
...
\begin{proof}
    \stars
\end{proof}
\begin{theorem}[Sylow第三定理-计数定理]\label{xldsdl}
    已知群$G$的阶为$p^lm$,其中$(p,m)=1,p$为素数,$l\geq1$.若记$k$为$G$的\Sl{p}的个数,则\begin{align*}
        &k\equiv1\mod p,&&k|m.
    \end{align*}
\end{theorem}
\begin{analysis}
...
\begin{proof}
    ...
    \end{align*}则\begin{align*}
        ...
        \iff&P\subset N_P(Q),
    \end{align*}又$Q\subset N_P(Q)$且$P,Q$都是$G$的\Sl{p},所以由推论\ref{p81tl2}可知$P=Q$,进而$|X_0|=1$.由推论\ref{p77tl4}可知\begin{align*}
        |X_0|\equiv|X|\mod p,
...
\end{definition}
\begin{definition}[剩余类的乘法]\label{uyldif}
...
\begin{theorem}
    ...
    \end{align*}即\begin{center}
        \begin{tikzcd}
            &  & S'                                            \\
            &  &                                               \\
S \arrow[rruu, "\phi" description] \arrow[rr, "\pi" description] \arrow[rruu, "\psi\pi" description, dashed, bend left] &  & S/\sim \arrow[uu, "\psi" description, dashed]
\end{tikzcd}
    \end{center}
\end{theorem}
\begin{proof}
